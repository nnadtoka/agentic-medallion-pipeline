...
- **Ingestion idempotency race.** The successful-batch check in `ingest_olist.py` runs before
  ...
  success" and both append the same rows. Fix: serialize each logical batch with a Postgres
  advisory lock and re-check success after acquiring it.
...
- **Historical snapshots leak future customers.** `agg_customer_lifetime_value.sql` and
  ...
  distorting historical population/retention analysis. Fix: restrict the customer universe to
